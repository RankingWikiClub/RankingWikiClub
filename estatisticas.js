...
renderizarCompeticoes = function(){
  ...
  if (typeof window.aplicarPesquisaCompeticoes === "function") competicoes = window.aplicarPesquisaCompeticoes(competicoes);
...
/* Pesquisa principal da página Competições */
(function(){
  const renderAnterior = window.renderizarCompeticoes || renderizarCompeticoes;
  window.aplicarPesquisaCompeticoes = function(lista){
    const termo = normalizarTextoBusca(document.getElementById("pesquisaCompeticoes")?.value || "");
    if (!termo) return lista;
    return lista.filter(c => normalizarTextoBusca([c.nome, c.tipo, c.categoria, c.abrangencia, c.local, c.pais, c.continente, c.estado].join(" ")).includes(termo));
  };
  document.addEventListener("DOMContentLoaded", () => {
    document.getElementById("pesquisaCompeticoes")?.addEventListener("input", () => (window.renderizarCompeticoes || renderizarCompeticoes)());
  });
})();
