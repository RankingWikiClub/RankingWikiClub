...
ALTER TABLE public.times ADD COLUMN IF NOT EXISTS ativo boolean DEFAULT true;
ALTER TABLE public.selecoes ADD COLUMN IF NOT EXISTS escudo_url text;
ALTER TABLE public.selecoes ADD COLUMN IF NOT EXISTS ativa boolean DEFAULT true;
ALTER TABLE public.competicoes ADD COLUMN IF NOT EXISTS logo_url text;
ALTER TABLE public.competicoes ADD COLUMN IF NOT EXISTS ativa boolean DEFAULT true;

UPDATE public.times SET ativo = true WHERE ativo IS NULL;
UPDATE public.selecoes SET ativa = true WHERE ativa IS NULL;
UPDATE public.competicoes SET ativa = true WHERE ativa IS NULL;
...
CREATE POLICY "futpedia_storage_select" ON storage.objects FOR SELECT USING (bucket_id IN ('escudos-times', 'escudos-selecoes', 'logos-competicoes'));
CREATE POLICY "futpedia_storage_insert" ON storage.objects FOR INSERT WITH CHECK (bucket_id IN ('escudos-times', 'escudos-selecoes', 'logos-competicoes'));
CREATE POLICY "futpedia_storage_update" ON storage.objects FOR UPDATE USING (bucket_id IN ('escudos-times', 'escudos-selecoes', 'logos-competicoes')) WITH CHECK (bucket_id IN ('escudos-times', 'escudos-selecoes', 'logos-competicoes'));
CREATE POLICY "futpedia_storage_delete" ON storage.objects FOR DELETE USING (bucket_id IN ('escudos-times', 'escudos-selecoes', 'logos-competicoes'));
