...
function renderizarResumoEstatisticas(competicoes, edicoes) {
  ...
  area.innerHTML = `
    ...
    <div class="ranking-mini"><h3>Maior campeão</h3>${maiorCampeao ? `<p>${linkParticipanteEstatistica(maiorCampeao.id)} - ${maiorCampeao.total} título(s)</p>` : `<p>Nenhum campeão cadastrado.</p>`}</div>
    <div class="ranking-mini"><h3>Maior vice</h3>${maiorVice ? `<p>${linkParticipanteEstatistica(maiorVice.id)} - ${maiorVice.total} vice(s)</p>` : `<p>Nenhum vice cadastrado.</p>`}</div>
  `;
}
