...
function prepararFiltrosEstatisticas() {
  ...
  if (categoria) {
    const valorAtual = categoria.value || "todas";
    categoria.innerHTML = `
      <option value="todas">Todas</option>
      <option value="clube">Clubes</option>
      <option value="selecao">Seleções</option>
    `;
    categoria.value = ["todas", "clube", "selecao"].includes(valorAtual) ? valorAtual : "todas";
  }

  if (abrangencia) {
    const valorAtual = abrangencia.value || "Mundial";
    abrangencia.innerHTML = `
      <option value="Mundial">Mundo</option>
      <option value="Continental">Continente</option>
      <option value="País">País</option>
    `;
    abrangencia.value = ["Mundial", "Continental", "País"].includes(valorAtual) ? valorAtual : "Mundial";
  }
...
function atualizarVisibilidadeFiltrosEstatisticas() {
  const categoria = document.getElementById("filtroCategoriaEstatisticas")?.value || "todas";
  const abrangencia = document.getElementById("filtroAbrangenciaEstatisticas")?.value || "Mundial";
  ...
  if (grupoAbrangencia) grupoAbrangencia.classList.toggle("oculto", !ehClubes);
  if (grupoLocal) grupoLocal.classList.toggle("oculto", !(ehClubes && abrangencia === "Continental"));
  if (grupoPais) grupoPais.classList.toggle("oculto", !(ehClubes && (abrangencia === "Continental" || abrangencia === "País")));
}
...
function obterFiltrosEstatisticas() {
  return {
    ...
    abrangencia: document.getElementById("filtroAbrangenciaEstatisticas")?.value || "Mundial",
...
function filtrarCompeticoesEstatisticas(banco, filtros) {
  ...
  const abrangencia = filtros.abrangencia || "Mundial";
  ...
  return (banco.competicoes || []).filter(c => {
    ...
    if (categoria === "todas") {
      return cat === "clube" || cat === "selecao";
    }

    if (cat !== categoria) return false;

    if (categoria === "selecao") return true;

    if (abrangencia === "Mundial") {
      return competicaoEhMundoEstatisticas(c);
    }

    if (abrangencia === "Continental") {
      if (!continente || !pais) return false;
...
function renderizarResumoEstatisticas(competicoes, edicoes) {
  ...
  area.innerHTML = `
    <div class="ranking-mini resumo-dashboard-card"><h3>Competições</h3><p>${competicoes.length}</p></div>
    <div class="ranking-mini resumo-dashboard-card"><h3>Edições</h3><p>${edicoes.length}</p></div>
    <div class="ranking-mini resumo-dashboard-card"><h3>Maior campeão</h3>${maiorCampeao ? `<p>${linkParticipanteEstatistica(maiorCampeao.id)}</p><small>${maiorCampeao.total} título(s)</small>` : `<p>Nenhum campeão cadastrado.</p>`}</div>
    <div class="ranking-mini resumo-dashboard-card"><h3>Maior vice</h3>${maiorVice ? `<p>${linkParticipanteEstatistica(maiorVice.id)}</p><small>${maiorVice.total} vice(s)</small>` : `<p>Nenhum vice cadastrado.</p>`}</div>
    <div class="ranking-mini resumo-dashboard-card"><h3>Maior finalista</h3>${maiorFinalista ? `<p>${linkParticipanteEstatistica(maiorFinalista.id)}</p><small>${maiorFinalista.finais} final(is)</small>` : `<p>Nenhum finalista cadastrado.</p>`}</div>
  `;
...
function renderizarGrupoRankingsCategoria(titulo, edicoesCategoria) {
  return `
    <div class="rankings-categoria-dashboard">
      ...
      <div class="rankings-estatisticas-grid rankings-categoria-grid">
        ${renderizarCardRankingCampeoes(edicoesCategoria)}
        ${renderizarCardRankingVices(edicoesCategoria)}
        ${renderizarCardRankingFinalistas(edicoesCategoria)}
      </div>
    </div>
  `;
}

function renderizarCardRankingCampeoes(edicoes) {
  const rankingCampeoes = contarRankingPorId((edicoes || []).map(e => e.campeaoId));
  return `
    <div class="ranking-mini ranking-dashboard-card">
      <h3>🥇 Ranking de Campeões</h3>
      ${rankingCampeoes.length ? `
        <ol class="lista-ranking-dashboard">
          ${rankingCampeoes.slice(0, 10).map(item => `
            <li>${linkParticipanteEstatistica(item.id)}<small>${item.total} título(s)</small></li>
          `).join("")}
        </ol>
...
function renderizarCardRankingVices(edicoes) {
  const rankingVices = contarRankingPorId((edicoes || []).map(e => e.viceId));
  return `
    <div class="ranking-mini ranking-dashboard-card">
      ...
      ${rankingVices.length ? `
        <ol class="lista-ranking-dashboard">
          ${rankingVices.slice(0, 10).map(item => `
            <li>${linkParticipanteEstatistica(item.id)}<small>${item.total} vice(s)</small></li>
          `).join("")}
        </ol>
...
function renderizarCardRankingFinalistas(edicoes) {
  const rankingFinalistas = contarRankingFinalistasEstatisticas(edicoes || []);
  return `
    <div class="ranking-mini ranking-dashboard-card">
      <h3>⭐ Ranking de Finalistas</h3>
      ${rankingFinalistas.length ? `
        <ol class="lista-ranking-dashboard">
          ${rankingFinalistas.slice(0, 10).map(item => `
            <li>
              ${linkParticipanteEstatistica(item.id)}
              <small>${item.finais} final(is) • ${item.titulos} título(s) • ${item.vices} vice(s)</small>
            </li>
          `).join("")}
        </ol>
      ` : `<p>Nenhum finalista cadastrado.</p>`}
    </div>
  `;
}
