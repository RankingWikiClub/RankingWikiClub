...
function listarParticipantesTitulo(banco) {
...
function carregarListasTitulo() {
  ...
  const participantes = listarParticipantesTitulo(banco);
  ...
  preencherSelect(
    ...
    "Selecione o campeão",
    item => item.id,
    item => `${item.nome} - ${item.tipo === "selecao" ? "Seleção" : "Clube"}${item.pais ? " - " + item.pais : ""}`
  );
