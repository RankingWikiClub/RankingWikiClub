...
async function fpEdicaoAtualizarTimeSql(id, dados) {
  const supa = fpEdicaoSqlCliente();
  if (!supa) return true;
  const paisId = await fpEdicaoBuscarPaisId(dados.pais);
  const payload = {
    nome_curto: dados.nomeCurto || dados.nome,
    nome: dados.nomeCompleto || dados.nomeCurto || dados.nome,
    cidade: dados.cidade || null,
    estado: dados.estado || null,
    fundacao: fpEdicaoNormalizarData(dados.fundacao),
    escudo_url: dados.escudo || null
  };
  if (paisId) payload.pais_id = paisId;
  const { error } = await supa.from("times").update(payload).eq("id", id);
...
async function fpEdicaoAtualizarSelecaoSql(id, dados) {
  const supa = fpEdicaoSqlCliente();
  if (!supa) return true;
  const paisId = await fpEdicaoBuscarPaisId(dados.pais || dados.nome);
  const payload = {
    nome: dados.nome || dados.pais || null,
    escudo_url: dados.escudo || null
  };
  if (paisId) payload.pais_id = paisId;
  const { error } = await supa.from("selecoes").update(payload).eq("id", id);
  if (error) throw error;
  return true;
}
...
async function fpEdicaoAtualizarCompeticaoSql(id, dados) {
  ...
  if (!supa) return true;
  const paisId = await fpEdicaoBuscarPaisId(dados.pais || "");
  const continenteId = await fpEdicaoBuscarContinenteId(dados.continente || "");
  const payload = {
    nome: dados.nome || null,
    tipo: dados.categoria === "selecao" ? "selecoes" : "clubes",
    abrangencia: fpEdicaoAbrangenciaParaSql(dados.abrangencia),
    logo_url: dados.escudo || null,
    ativa: true
  };
  if (paisId) payload.pais_id = paisId;
  else payload.pais_id = null;
  if (continenteId) payload.continente_id = continenteId;
  else payload.continente_id = null;
  const { error } = await supa.from("competicoes").update(payload).eq("id", id);
  if (error) throw error;
  return true;
}
