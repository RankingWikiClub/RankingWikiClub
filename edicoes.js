  ...
  conteudo.innerHTML = `
    ...
    ${
      titulosPorCompeticao.length
        ? `
          <div class="tabela-container">
            <table class="tabela-detalhe-time tabela-titulos-compacta">
              <thead>
                <tr>
                  <th>Competição</th>
                  <th class="th-total-titulos">Total de títulos</th>
                </tr>
              </thead>
                ...
                ${titulosPorCompeticao.map(item => `
                  <tr>
                    ...
                    <td class="total-titulos-com-estrelas">
                      <div class="numero-total-titulos">${item.total}</div>
                      <div class="titulo-estrelas">${"★".repeat(item.total)}</div>
                    </td>
                  </tr>
                `).join("")}
              </tbody>
...
function abrirDetalhesLiga(id) {
  ...
  conteudo.innerHTML = `
    ...
    <div id="listaEdicoesCompeticaoDetalhe">
      ${
        edicoes.length
          ? edicoes.map(e => `<p class="linha-edicao-competicao-detalhe">${limparTexto(e.ano)} - Campeão: ${linkTimeCompeticaoDetalhe(e.campeaoId)} | Vice: ${linkTimeCompeticaoDetalhe(e.viceId)}</p>`).join("")
          : `<p>Nenhuma edição cadastrada.</p>`
      }
    </div>

    <br>
    <h3>Maior finalista</h3>
    ${
      maiorFinalista
        ? `<p>${linkTimeCompeticaoDetalhe(maiorFinalista.id)} - ${maiorFinalista.finais} final(is) • ${maiorFinalista.titulos} título(s) • ${maiorFinalista.vices} vice(s)</p>`
    ...
    ${
      rankingFinalistas.length
        ? rankingFinalistas.map(r => `<p>${linkTimeCompeticaoDetalhe(r.id)} - ${r.finais} final(is) • ${r.titulos} título(s) • ${r.vices} vice(s)</p>`).join("")
        : `<p>Nenhum finalista cadastrado.</p>`
    }
...
function abrirDetalhesSelecao(id) {
  ...
  const titulosPorCompeticao = contarTitulosPorCompeticao(titulosSelecao, banco);
